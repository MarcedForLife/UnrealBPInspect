...
pub fn lower_array_get_out_to_assignment(body: &mut [Stmt]) {
    // Preorder is safe here: try_rewrite only matches single-OUT array-get
    // Call statements, which carry no child bodies, and statements that DO
    // have children never match, so traversal order cannot affect the result.
